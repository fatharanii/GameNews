...
app.delete ('/api/read_later/news/:id', async (req, res)=>{
    await read_later.deleteNewsFromBookmark(req.params.id)
    res.status(200).send()
})
...
app.get('/api/read_later/search/:judul_berita', async(req,res)=>{
    const news = await read_later.getNewsByJudul(req.params.judul_berita)
